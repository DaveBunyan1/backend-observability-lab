...
def get_jobs(request: Request):
    ...
    print(f"{request_id} Received request: GET /jobs")
...
def get_job(job_id: str, request: Request) -> Job:
    request_id = request.state.request_id
    print(f"{request_id} Received request: GET /jobs/{job_id}")
    for job in FAKE_JOBS:
        if job.job_id == job_id:
            print(f"{request_id} Found job: {job_id}")
            return job
    print(f"{request_id} Job not found: {job_id}")
...
def create_job(job: Annotated[Job, Body()], request: Request):
    request_id = request.state.request_id
    print(f"{request_id} Received request: POST /jobs")
    FAKE_JOBS.append(job)
    print(f"{request_id} Job created successfully: {job}")
    return job
...
def delete_job(job_id: str, request: Request):
    request_id = request.state.request_id
    print(f"{request_id} Received request: DELETE /jobs/{job_id}")
    for job in FAKE_JOBS:
        if job.job_id == job_id:
            FAKE_JOBS.remove(job)
            print(f"{request_id} Deleted job: {job_id}")
            return job
    print(f"{request_id} Job not found: {job_id}")
...
def update_job(job_id: str, job_message: Annotated[str, Body()], request: Request):
    request_id = request.state.request_id
    print(f"{request_id} Received request: PUT /jobs/{job_id}")
    for job in FAKE_JOBS:
        if job.job_id == job_id:
            job.job_message = job_message
            print(f"{request_id} Updated job: {job_id} with message: {job_message}")
            return job
    print(f"{request_id} Job not found: {job_id}")
    raise HTTPException(status_code=404, detail="Job not found")
